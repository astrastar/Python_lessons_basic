    ...
    def __repr__(self):
        ...
        for index in range(0, 3):
            if len (str(print_card[0][index])) == 1:
                print_card[0][index] = str(f' {print_card[0][index]}')
...
class Kegs:
    ...
    def ge_text_about_keg_curr(self):
        return f'\nНовый бочонок {self.keg_curr}, осталось {len(self.kegs_list)}'
...
class Game():
    ...
    def __repr__(self):
        return f'{self.kegs.ge_text_about_keg_curr()}\n{self.player_card}\n{self.computer_card}'
    ...
    def get_result_step(self):
        ...
        if input_data == 'Y' or input_data == 'y':
            ...
            print('Нет такой команды, Y или N?')
            self.status_game = -1
